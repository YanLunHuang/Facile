...
#HOST_SRCS += src/host_program_old/host_X_Nbuffer_3CU.cpp
HOST_SRCS += src/host_program/host_4HBM.cpp
...
$(TEMP_DIR)/alveo_hls4ml.xo: src/alveo_hls4ml.cpp
	mkdir -p $(TEMP_DIR)
	$(VPP) $(CLFLAGS) --temp_dir $(TEMP_DIR) -c -k alveo_hls4ml -I'$(<D)' -o'$@' '$<' ./src/ereg_v1.cpp -I./src/ -I./src/weights -I./src/nnet_utils/ --config config.ini
$(BUILD_DIR)/alveo_hls4ml.xclbin: $(BINARY_CONTAINER_alveo_hls4ml_OBJS)
	mkdir -p $(BUILD_DIR)
	$(VPP) $(CLFLAGS) --temp_dir $(BUILD_DIR) -l $(LDCLFLAGS) -o'$@' $(+) --config config.ini --kernel_frequency 250
